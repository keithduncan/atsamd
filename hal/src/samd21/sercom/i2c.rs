...
macro_rules! i2c {
    ($Type:ident: ($sercom:ident, $SERCOM:ident, $powermask:ident, $clock:ident)) => (
        $crate::paste::item! {
            ...
            pub struct [<$Type Padout>]<SDA, SCL> {
                _sda: SDA,
                _scl: SCL,
            }
            ...
            impl<PIN0, PIN1> From<([<$sercom Pad0>]<PIN0>, [<$sercom Pad1>]<PIN1>)> for [<$Type Padout>]<[<$sercom Pad0>]<PIN0>, [<$sercom Pad1>]<PIN1>> {
                fn from(pads: ([<$sercom Pad0>]<PIN0>, [<$sercom Pad1>]<PIN1>)) -> [<$Type Padout>]<[<$sercom Pad0>]<PIN0>, [<$sercom Pad1>]<PIN1>> {
                    [<$Type Padout>] { _sda: pads.0, _scl: pads.1 }
                }
            }
            ...
            pub struct $Type<SDA, SCL> {
                padout: [<$Type Padout>]<SDA, SCL>,
                sercom: $SERCOM,
            }

            impl<SDA, SCL> $Type<SDA, SCL> {
                ...
                /// );
                /// ```
                pub fn new<F: Into<Hertz>, T: Into<[<$Type Padout>]<SDA, SCL>>>(
                    clock: &clock::$clock,
                    freq: F,
                    sercom: $SERCOM,
                    pm: &mut PM,
                ...
                pub fn free(self) -> ([<$Type Padout>]<SDA, SCL>, $SERCOM) {
            ...
            impl<SDA, SCL> Write for $Type<SDA, SCL> {
                type Error = I2CError;
            ...
            impl<SDA, SCL> Read for $Type<SDA, SCL> {
                type Error = I2CError;
            ...
            impl<SDA, SCL> WriteRead for $Type<SDA, SCL> {
                type Error = I2CError;
